# Dockerfile for serving a PI policy on AMD ROCm GPUs.
# Based on UV's instructions: https://docs.astral.sh/uv/guides/integration/docker/#developing-in-a-container

# Build the container:
# docker build . -t openpi_server_rocm -f scripts/docker/serve_policy_rocm.Dockerfile

# Run the container (requires ROCm driver on host):
# docker run --rm -it --network=host -v .:/app --device=/dev/kfd --device=/dev/dri --group-add video openpi_server_rocm /bin/bash
...
RUN /.venv/bin/pip install torch --index-url https://download.pytorch.org/whl/rocm6.2
